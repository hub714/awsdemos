...
bucketPath = 'sm-groundtruth-text'
localPath = 'text'
...
# Lists the local directory (text by default), creates an outputJSON with the proper
# source-ref formatting, then uploads the object to S3

for f in listdir(localPath):
    textFile = open(localPath+"/"+f, "r")
    for line in textFile:
        outputJSON.append({'source': line})
...
response = uploadObject('annotation-tool/template-ner.liquid', bucketName, bucketPath+'/annotation-tool/template-ner.liquid')
